...
try {
  ...
  await page.waitForFunction(async () => (await navigator.serviceWorker.getRegistration('/'))?.active?.scriptURL.endsWith('/mobile-workbench/sw.js'))
  assert.deepEqual(await page.evaluate(() => caches.keys()), [])
  const manifest = await page.evaluate(async () => await (await fetch('/mobile-workbench/manifest.webmanifest', { credentials: 'same-origin' })).json())
