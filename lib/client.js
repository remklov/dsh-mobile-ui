...
var SHELL_MANIFEST_PATH = "/manifest.webmanifest";
function isShellManifest(href, origin) {
  try {
...
function isReplaceableManifest(href, origin) {
...
@media (max-width: 767px) {
  ...
  /* The bar deliberately STAYS while the details panel is open. Hiding it
     removed the only way back: the native expand control is hidden on phones,
     so the panel became a dead end. The \u25A3 button doubles as its close. */
  ...
  [data-mwb-frame] [data-sidebar-right-panel="fullscreen"] {
    box-sizing: border-box;
    /* Clear the bar, which stays on top of the panel. --mwb-bar-height already
       includes the safe-area inset, so it must not be added a second time. */
    padding-top: var(--mwb-bar-height);
    padding-right: env(safe-area-inset-right, 0px);
    padding-bottom: env(safe-area-inset-bottom, 0px);
    padding-left: env(safe-area-inset-left, 0px);
...
function MobileWorkbench({ layout }) {
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { ref: root, "data-mwb-controls": "", children: state.mobile && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { "data-mwb-bar": "", children: [
      ...
      rightPanelAvailable && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
        "button",
        {
          type: "button",
          "data-mwb-button": "",
          "data-mwb-open-details": "",
          "aria-expanded": state.detailsOpen,
          "aria-label": state.detailsOpen ? "Close file and details panel" : "Open file and details panel",
          onClick: () => {
            const frame = adapter.current?.frame;
            const pick = (selector) => frame?.querySelector(selector) ?? document.querySelector(selector);
            const order = state.detailsOpen ? ["[data-sidebar-right-toggle]", "[data-sidebar-right-expand]"] : ["[data-sidebar-right-expand]", "[data-sidebar-right-toggle]"];
